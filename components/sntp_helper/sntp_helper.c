...
void time_sync_notification_cb(struct timeval* tv) { ESP_LOGI(TAG, "Time has been synchronized with NTP!"); }
...
bool time_is_stale(time_t now) {
  ...
  ESP_LOGI(TAG, "The last sync with network time was: %s, which was %.2f seconds ago", strftime_buf,
